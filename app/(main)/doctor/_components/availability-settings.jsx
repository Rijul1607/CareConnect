...
const {loading, fn:submitSlots,data}=useFetch(setAvailabilitySlots);
  ...
  return (
    <Card>
  <CardHeader>
    <CardTitle className="text-xl font-bold flex items-center"> <Clock className="h-5 w-5 mr-2 text-emerald-400"/>  Availability Settings</CardTitle>
  ...
  <CardContent>
    {!showForm ? (<>
      <div className='mb-6  '>
        ...
        {slots.length === 0 ? (
                ...
                <div className="space-y-3 shadow-[0_0_10px_#34d399] backdrop-blur-sm rounded-md ">
                  {slots.map((slot) => (
                    <div
                      key={slot.id}
    ...
    <h3 className='text-lg font-medium mb-2'>Set Daily Availability</h3>
    <div className='grid grid-cols-1 sm:grid-cols-2 gap-4'>
      <div>
        ...
        />
        {errors.startTime && (<p className='text-sm font-medium text-red-500'>{errors.startTime.message}</p>)}
      </div>
      <div>
        <Label htmlFor="endTime" className='mb-2'>End Time</Label>
        <Input id="endTime" type="time" {...register("endTime",{
          required:"End time is required",
        })}
        className="bg-background"
        />
        {errors.endTime && (<p className='text-sm font-medium text-red-500'>{errors.endTime.message}</p>)}
      </div>
    </div>
    ...
    <div className='flex justify-end space-x-3 pt-2'>
      ...
      <Button
      type="submit" disabled={loading} className="bg-emerald-600 hover:bg-emerald-700">
        {loading ?(<> <Loader2 className='mr-2 h-4 w-4 animate-spin'/> Saving... </>):("Save Availability")}
